...
# Change: Updated vendor match logic to first 7 characters — 2025-05-08 02:16 AM
...
def normalize_and_reconcile(bank_file, finance_file, log_callback=None, log_save_button=None):
    try:
        ...
        bank_df['norm_amount'] = bank_df['Withdrawal (Dr.)'].apply(lambda x: round(float(str(x).replace(',', '')), 2) if pd.notna(x) else 0.0)
        bank_df['norm_vendor'] = bank_df['der_bank_ven'].apply(lambda x: str(x).strip().upper())

        finance_df['norm_date'] = finance_df['Payment Date'].apply(safe_excel_to_date)
        finance_df['norm_amount'] = finance_df['Credit Amount'].apply(lambda x: round(float(str(x).replace(',', '')), 2) if pd.notna(x) else 0.0)
        finance_df['norm_vendor'] = finance_df['der_fin_ven'].apply(lambda x: str(x).strip().upper())
        ...
        for i, b_row in bank_df.iterrows():
            ...
            for j, f_row in finance_df.iterrows():
                ...
                if (
                    ...
                    b_row['norm_vendor'][:7] == f_row['norm_vendor'][:7]
                ):
            ...
            # 1-to-2
            candidate_finance = finance_df[(finance_df['norm_date'] == b_row['norm_date']) & (finance_df['norm_vendor'].str[:7] == b_row['norm_vendor'][:7])]
            ...
            for n in range(3, 11):
                ...
                candidate_finance_n = finance_df[(finance_df['norm_date'] == b_row['norm_date']) & (finance_df['norm_vendor'].str[:7] == b_row['norm_vendor'][:7])]
                for combo in combinations(candidate_finance_n.index.intersection(unmatched_finance_indexes), n):
